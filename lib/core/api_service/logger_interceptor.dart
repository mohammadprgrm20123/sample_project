...
class LoggerInterceptor extends Interceptor {
  ...
  void _printBoxed({final String? header, final String? text}) {
    logPrint('');
    logPrint('╔╣ $header');
    logPrint('║  $text');
    _printLine('╚');
  ...
  void _printResponseHeader(final Response response) {
  ...
  void _printKV(final String? key, final Object? v) {
    final pre = '╟ $key: ';
  ...
  void _printPrettyMap(
    ...
    data.keys.toList().asMap().forEach((final index, final dynamic key) {
  ...
  void _printList(final List list, {final int tabs = initialTab}) {
    list.asMap().forEach((final i, final dynamic e) {
  ...
  bool _canFlattenMap(final Map map) =>
      map.values
          .where((final dynamic val) => val is Map || val is List)
          .isEmpty &&
      map.toString().length < maxWidth;

  bool _canFlattenList(final List list) =>
      list.length < 10 && list.toString().length < maxWidth;

  void _printMapAsTable(final Map? map, {final String? header}) {
